############
# R script to run classical
# model checks on the fitted species richness
...
# then for the abundance
ppd_abund <- lapply(m_list[1:9], function(x) brms::pp_check(x, type = "dens_overlay", ndraws = 100) + scale_x_sqrt())
gg_abund <- gridExtra::grid.arrange(gridExtra::arrangeGrob(grobs=ppd_abund,
                                                           nrow=3, ncol=3))
# save the plot
ggsave("figures/XX_ppd_rich_tn.png", gg_rich)
ggsave("figures/XX_ppd_abund_zinb.png", gg_abund)
...
cc_d$var <- rep(c("edge", "field", "low", "rich"), each = 300 * 3)
cc_d$var <- factor(cc_d$var, levels = c("rich", "edge", "field", "low"),
                   labels = c("Species richness", "Edge-breeders", "Field-breeders",
                              "Foraging visitors"))
cc_d$year <- rep(rep(c(2007, 2010, 2016), each = 300), 4)

cc_d %>%
  filter(x < 1e5) %>%
  mutate(x = x / 1e3) -> cc_f

cc_f$var <- factor(cc_f$var, labels = c("Edge-breeding",
                                        "Field-breeding",
                                        "Shrub-breeding",
                                        "Species richness"))
...
gg_spat <- ggplot(cc_f, aes(x=x, y=corr, ymin=lci, ymax=hci)) +
  ...
  geom_path() +
  facet_grid(rows = vars(var), cols = vars(year)) +
